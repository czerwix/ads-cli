...
public struct AndroidDocsProvider: DocsProvider {
    public let source = "android"
    ...
    public func doc(pathOrURL: String, client: HTTPClient) async throws -> DocumentPage {
        let url = URL(string: pathOrURL.hasPrefix("http") ? pathOrURL : "https://developer.android.com/\(pathOrURL)")!
        let response = try await client.get(url: url)
        let html = String(decoding: response.data, as: UTF8.self)
    ...
    public static func parseSearchHTML(_ html: String, limit: Int) -> [SearchResult] {
        SearchHTMLParser.parse(html: html, baseURL: "https://developer.android.com", source: "android", limit: limit)
    }
}
